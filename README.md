...
| Item | Template (Consultoria Suk) | Esta LP (Diagnóstico Advocacia) |
|------|----------------------------|---------------------------------|
...
| `RD_CONVERSION_IDENTIFIER` | `lp-consultoria-suk` | **`lp-diagnostico-advocacia`** |
...
3. **Dores** — 4 cards com ícone (planilha de prazos, retrabalho, portal do cliente, dados espalhados).
...
| `RD_CONVERSION_IDENTIFIER` | ⬜ | Identificador da conversão (padrão: `lp-diagnostico-advocacia`) |

Mapeamento enviado ao RD: `name` (nome), `email`, `mobile_phone` (WhatsApp), `company_name` (nome do escritório), `job_title` (cargo), `tags` (`diagnostico-projeto`, `nicho-advocacia`), `available_for_mailing` e `legal_bases` (consentimento LGPD quando marcado). Após alterar variáveis no Vercel, faça um **Redeploy**.

> **GTM** (`GTM-N384CG7L`) segue instalado nas duas páginas; o evento `conversao_lead` (com `conversao_identificador: "lp-diagnostico-advocacia"`) dispara só na thank you page, após a conversão real.
